...
import { billData } from "./data/billData.js";
...
function generateBillPDF(filePath, billData) {
  ...
  doc
    ...
    .text(`${billData.company.name}`, 105, 20, { align: "center" });
  doc
    .setFontSize(10)
    .setFont("helvetica", "bold")
    .text(`${billData.company.address}`, 105, 28, { align: "center" });
  ...
  y += 8;
  doc.setFontSize(10).setFont("helvetica", "bold");
  doc.text(`Discount : ${currency(discountAmount)}`, 191, y, {
    align: "right",
  });
  doc.setFontSize(10).setFont("helvetica", "bold");
  y += 8;
  ...
  doc.setFontSize(10).setFont("helvetica", "bold");
  y += 8;
  doc.text(`Balance Due : ${currency(balanceAmount)}`, 190, y, {
    align: "right",
  });
  doc.setFontSize(10).setFont("helvetica", "bold");
...
app.get("/download-pdf", (req, res) => {
  ...
  console.log("Generate_PDF", filePath);
...
app.get("/", (req, res) => {
  res.status(200).send("Generate PDF using Node");
});
